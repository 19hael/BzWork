...
export default function Home() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.25),_transparent_55%)]">
      <div className="absolute inset-0 bg-[linear-gradient(135deg,_rgba(15,23,42,0.9),_rgba(8,11,27,0.95))]" />
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 py-16 text-slate-100 lg:flex-row lg:items-center lg:gap-16">
        <div className="flex-1 space-y-8">
          <p className="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-1 text-[11px] font-semibold uppercase tracking-[0.4em] text-cyan-300/90">
            BzWork Demo
          ...
          <ul className="space-y-3">
            {heroHighlights.map((item) => (
              <li key={item} className="flex items-center gap-3 text-sm text-white/70">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10 text-cyan-300">
          ...
          <div className="flex flex-wrap gap-4">
            <Link
              href="/dashboard"
              className="inline-flex items-center gap-2 rounded-2xl bg-cyan-400/90 px-6 py-3 font-semibold text-slate-900 shadow-lg shadow-cyan-400/30"
            >
              Explore dashboard
      ...
      <section className="relative mx-auto grid w-full max-w-6xl gap-4 px-6 pb-16 text-sm text-white/80 md:grid-cols-2">
        ...
        <div className="rounded-3xl border border-white/20 bg-gradient-to-br from-cyan-500/20 via-transparent to-indigo-500/30 p-6">
